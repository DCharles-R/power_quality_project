...
class EspectrogramaSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        exclude = ['data_espectrograma']

class AnotacionSerializer(serializers.ModelSerializer):
    usuario_creacion_nombre = serializers.StringRelatedField(source='usuario_creacion.username', read_only=True)
    # usuario_anotador_username = serializers.CharField(source='usuario_anotador.username', read_only=True)

    class Meta:
        model = Anotacion
        fields = '__all__'
        read_only_fields = ('usuario_creacion', 'fecha_anotacion')
