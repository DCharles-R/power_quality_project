...
def procesar_evento_completo_task(muestra_id: int): # <<-- CAMBIO 1: Recibe muestra_id (entero)
    ...
    try:
        ...
        logger.info(f"[{timezone.now()}] Iniciando procesamiento ASÍNCRONO para Muestra ID: {muestra.id} (Event ID: {event_id})...")

        # <<-- CAMBIO 3: La verificación ahora es sobre el estado de esta muestra específica
        if muestra.estado_procesamiento == 'procesado':
            logger.warning(f"[{timezone.now()}] La muestra con ID {muestra.id} ya está en estado 'procesado'. Omitiendo.")
            return None

    except Muestra.DoesNotExist:
        logger.error(f"[{timezone.now()}] Error: No se encontró una muestra con el ID {muestra_id}.")
        return None
    ...
    try:
        ...
        if not puntos_signal or len(puntos_signal) < 5120:
            logger.error(f"[{timezone.now()}] No se encontraron suficientes datos para el event_id '{event_id}'.")
            muestra.estado_procesamiento = 'error' # Marcar como error
            muestra.save()
            return None
    except Exception as e:
        logger.error(f"[{timezone.now()}] Error al obtener datos de InfluxDB para '{event_id}': {e}")
        muestra.estado_procesamiento = 'error' # Marcar como error
        muestra.save()
        return None
    ...
    try:
        logger.info(f"[{timezone.now()}] Calculando la Transformada de Stockwell...")
        matriz_espectrograma = mst_processing(valores)
        espectrograma_bytes = pickle.dumps(matriz_espectrograma)

    except Exception as e:
        logger.error(f"[{timezone.now()}] Error al calcular la Transformada de Stockwell: {e}")
        muestra.estado_procesamiento = 'error' # Marcar como error
        muestra.save()
        return None
    ...
    try:
        logger.info(f"[{timezone.now()}] Guardando el espectrograma en PostgreSQL...")
        ...
        logger.error(f"[{timezone.now()}] Error al guardar el Espectrograma en PostgreSQL: {e}")
    ...
    logger.info(f"[{timezone.now()}] ✅ Procesamiento ASÍNCRONO de la Muestra ID '{muestra.id}' completado exitosamente.")
