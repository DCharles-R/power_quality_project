...
class InfluxService:
    ...
    def get_recent_event_ids(self, time_range_minutes: int = 5):
        """
        Recupera los event_ids únicos que han aparecido en los últimos X minutos.
        """
        logger.info(f"Buscando event_ids únicos en los últimos {time_range_minutes} minutos...")

        query = f'''
        from(bucket: "{self.bucket}")
          |> range(start: -{time_range_minutes}m) // Rango de tiempo dinámico
          |> filter(fn: (r) => r._measurement == "voltage_waveform")
          |> keep(columns: ["event_id"]) // Solo nos interesa la columna del tag event_id
          |> distinct(column: "event_id") // Obtiene los valores únicos
          |> yield(name: "unique_events")
        '''
        logger.debug(f"Consulta para IDs recientes:\n{query}")
        ...
        try:
            ...
            for table in tables:
                for record in table.records:
                    # El valor del tag se encuentra en el campo 'event_id' del registro
                    if "event_id" in record.values:
                        event_ids.append(record.values["event_id"])

            logger.info(f"Se encontraron {len(event_ids)} event_ids únicos recientes.")
            return event_ids
        except Exception as e:
            logger.error(f"Error al obtener event_ids recientes de InfluxDB: {e}", exc_info=True)
            return []
